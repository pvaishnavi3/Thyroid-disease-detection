...
main = tkinter.Tk()
main.title("Thyroid Disease Detection using Machine Learning Algorithms") 
...
def graph():
    # Check if all metric arrays have at least 3 values
    if len(accuracy) < 3 or len(precision) < 3 or len(recall) < 3 or len(fscore) < 3:
        text.insert(END, "Please run all algorithms (Naive Bayes, SVM, and Random Forest) before plotting the graph.\n")
        return

    # Create DataFrame with metrics for each algorithm
    df = pd.DataFrame([
        ['Precision', 'Naive Bayes', precision[0]],
        ['Recall', 'Naive Bayes', recall[0]],
        ['F1 Score', 'Naive Bayes', fscore[0]],
        ['Accuracy', 'Naive Bayes', accuracy[0]],
        ['Precision', 'SVM', precision[1]],
        ['Recall', 'SVM', recall[1]],
        ['F1 Score', 'SVM', fscore[1]],
        ['Accuracy', 'SVM', accuracy[1]],
        ['Precision', 'Random Forest', precision[2]],
        ['Recall', 'Random Forest', recall[2]],
        ['F1 Score', 'Random Forest', fscore[2]],
        ['Accuracy', 'Random Forest', accuracy[2]],
    ], columns=['Metric', 'Algorithm', 'Value'])

    # Pivot the DataFrame so that Metrics are the index and Algorithms are columns
...
font = ('times', 16, 'bold')
title = Label(main, text='Thyroid Disease Detection using Machine Learning Algorithms')
